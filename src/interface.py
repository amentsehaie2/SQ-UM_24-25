...
from operations import (
    ...
    list_users, update_own_password_service_engineer
)
...
def system_administration_menu(current_user):
    while True:
        ...
        print("6. Terug")
        choice = get_int_input("Select an option (1-6): ", 1, 6, current_user)
        if choice is None:
            break
...
def service_engineer_menu(current_user):
    while True:
        ...
        print("4. Uitloggen")
        choice = get_int_input("Select an option (1-4): ", 1, 4, current_user)
        if choice is None:
            break
        if choice == 1:
            search_scooters(current_user)
            ...
            update_own_password_service_engineer(current_user)
        elif choice == 4:
            print("Logging out...")
            break
        else:
